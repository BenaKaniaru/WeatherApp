export default function FeelsLike({ currentWeatherData }) {
  ...
  if (currentWeatherData.feelslike_c < currentWeatherData.temp_c) {
    ...
    } else if (currentWeatherData.wind_kph < 5 && condition.includes("sunny")) {
      feelsLikeReason = "Feels warmer due to direct sunlight and low wind.";
    } else {
      currentWeatherData.feelsLikeReason =
        "Feels slightly warmer than the actual temperature.";
    }
  } else {
    currentWeatherData.feelsLikeReason =
  ...
  return (
    <div className="text-white bg-gray-800 rounded-lg p-4 shadow-2xl">
      ...
      <div className="flex justify-between items-end">
        <div>
          <img src="/temparature.png" alt="Temparature Icon" />
        ...
        <span className="text-xs ml-6 font-light">{feelsLikeReason}</span>
      </div>
    </div>
  );
}
